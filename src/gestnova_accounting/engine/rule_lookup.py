...
class RuleLookup:
    ...
    def get(self, country: str, key: str, on_date: date) -> Rule:
        ...
        for rule in candidates:  # already sorted desc by effective_from
            if rule.effective_from <= on_date and (
                rule.effective_until is None or on_date <= rule.effective_until
            ):
                return rule
        raise NoRuleApplicable(
            country, key, on_date,
            hint=f"No vigencia covers {on_date.isoformat()}. Most recent rule starts {candidates[0].effective_from.isoformat()}."
        )
